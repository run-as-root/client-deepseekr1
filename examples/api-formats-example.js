...
async function demonstrateApiFormats() {
  ...
  try {
    ...
    try {
      const deepSeekResponse = await deepSeekClient.generate('What is the capital of France?');
      console.log('✅ DeepSeek Response:', deepSeekResponse.response?.substring(0, 100) + '...');
    } catch (error) {
    ...
    try {
      const openaiResponse = await openaiClient.generate('What is the capital of France?');
      console.log('✅ OpenAI Response:', openaiResponse.response?.substring(0, 100) + '...');
    } catch (error) {
    ...
    const autoClient = new DeepSeekClient();
    ...
    const switchableClient = new DeepSeekClient({
    ...
    const templateClient = new DeepSeekClient();

    try {
      const templateResponse = await templateClient.generateFromTemplate('explain-concept', {
      ...
      console.log('✅ Template Response Format:', templateClient.getApiFormat());
      console.log('Response preview:', templateResponse.response?.substring(0, 150) + '...');
    } catch (error) {
    ...
    const streamClient = new DeepSeekClient();
    ...
    const errorClient = new DeepSeekClient();
...
DEEPSEEK_TIMEOUT=30000`
  ...
  for (const [key, example] of Object.entries(examples)) {
    ...
    console.log(`const client = new DeepSeekClient(${JSON.stringify(example.config, null, 2)});`);
    console.log('```');
  ...
  for (const format of formats) {
    ...
    try {
      const client = new DeepSeekClient({
        apiFormat: format,
        baseUrl: format === 'openai' ? 'https://api.openai.com' : 'http://localhost:8080',
        token: 'test-token',
...
async function runAllExamples() {
  ...
  console.log('• DeepSeek API format (original, backward compatible)');
  console.log('• OpenAI API format (new, widely compatible)');
