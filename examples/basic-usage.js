...
function calculateTotal(items) {
  ...
  return total;
    ...
    const codeReviewResponse = await client.generateFromTemplate('code-review', {
      language: 'javascript',
...
async function advancedExamples() {
  try {
    ...
    const response1 = await client.generate(prompt, { model: 'deepseek-r1:8b' });
    ...
    const variables = [...new Set((template.match(/\{\{(\w+)\}\}/g) || []).map(match => match.slice(2, -2)))];
    ...
    if (perfResponse.total_duration) {
      console.log(`Server processing time: ${Math.round(perfResponse.total_duration / 1000000)}ms`);
    }
...
async function runAllExamples() {
  console.log('🎯 DeepSeek Client Examples');
  console.log('═'.repeat(50));
